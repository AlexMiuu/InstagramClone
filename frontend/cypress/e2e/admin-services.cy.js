...
describe('Admin Services', () => {
  const admin = {
    email: 'bbb', // Use a known admin from your DB
    password: 'bbb'
  };
  let token = '';

  before(() => {
    cy.request('POST', 'http://localhost:8080/auth/login', admin).then((resp) => {
      token = resp.body;
    });
  });
  ...
  // Add more admin-only tests (ban user, delete user, create tag, etc.)
});