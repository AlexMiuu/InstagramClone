...
describe('Auth API', () => {
  ...
  it('registers a new user', () => {
    cy.request('POST', 'http://localhost:8080/auth/register', user)
      .its('status').should('eq', 201);
  });

  it('logs in with the registered user', () => {
    cy.request('POST', 'http://localhost:8080/auth/login', {
      email: user.email,
      password: user.password
    }).then((resp) => {
      expect(resp.status).to.eq(200);
      expect(resp.body).to.be.a('string'); // JWT token
      cy.wrap(resp.body).as('userToken');
    });
  });
});